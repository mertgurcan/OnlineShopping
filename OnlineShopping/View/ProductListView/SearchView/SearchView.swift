//
//  SearchView.swift
//  OnlineShopping
//
//  Created by Mert Gürcan on 13.12.2020.
//

import UIKit

class SearchView: UIView {

    @IBOutlet weak var shadowLayer: UIView!
    @IBOutlet weak var backgroundView: UIView!
    @IBOutlet weak var searchBar: UISearchBar!
    @IBOutlet weak var patternView: PatternView!

    override init(frame: CGRect) {
        super.init(frame: frame)
        commonInit()
        updateUI()
    }
    required init?(coder: NSCoder) {
        super.init(coder: coder)
        commonInit()
        updateUI()
    }

    func commonInit(){
        let viewFromXib = Bundle.main.loadNibNamed("SearchView", owner: self, options: nil)![0] as! UIView
        viewFromXib.frame = self.bounds
        addSubview(viewFromXib)
    }
    func updateUI(){
        backgroundView.layer.masksToBounds = true
        backgroundView.layer.cornerRadius = 11
        shadowLayer.layer.cornerRadius = 11
        shadowLayer.layer.shadowColor = UIColor.black.cgColor
        shadowLayer.layer.shadowOpacity = 0.13
        shadowLayer.layer.shadowOffset = .zero
        shadowLayer.layer.shadowRadius = 3
        searchBar.searchTextField.backgroundColor = UIColor.white.withAlphaComponent(1)
        searchBar.layer.borderWidth = 1
        searchBar.layer.borderColor = UIColor.white.cgColor
    }

}
